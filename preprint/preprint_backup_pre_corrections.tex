...
\title{\textbf{Gradient-Based Pulse Optimization for Quantum Gates: A Verified GRAPE Implementation with Hardware-Representative Noise Models}}
...
\begin{figure}[t]
    ...
    \caption{Architecture diagram of the QubitPulseOpt hardware-calibrated workflow. Live calibration data is asynchronously queried from the IQM Resonance platform, used to instantiate a device-specific Lindblad simulation (``digital twin''), and fed into the GRAPE optimizer to produce hardware-tailored control pulses.}
    \label{fig:workflow}
\end{figure}
...
To evaluate the practical benefit of hardware-calibrated optimization, we instantiated the Lindblad simulation using representative calibration parameters consistent with the IQM Resonance platform architecture. The specific qubit examined (QB1) exhibited the following parameters:
%
\begin{itemize}
    \item T$_1$ = 50 $\mu$s
    \item T$_2$ = 70 $\mu$s
