\documentclass[twocolumn,preprintnumbers,amsmath,amssymb,pra,floatfix,longbibliography]{revtex4-2}
...
The robustness results (Fig.~\ref{fig:robustness}) show that GRAPE has good amplitude robustness: even at $\pm 5\%$ amplitude error, the minimum fidelity remains 0.994, the best among all three methods. DRAG also shows strong amplitude robustness (minimum 0.990), significantly outperforming Gaussian (minimum 0.965).
...
The error budget (Table~\ref{tab:error_budget}) shows a hierarchy for the three-level model at \SI{20}{\nano\second}:
...
\item \textit{Analytical DRAG only.} We use the analytical $\beta$ formula rather than numerical optimization of $\beta$. Numerically optimized DRAG~\cite{theis2018counteracting, motzoi2011backaction} could further improve performance, though it would still be limited to the functional form of Eq.~\eqref{eq:drag}.
\end{itemize}
...
We have presented a systematic comparison of Gaussian, DRAG, and GRAPE control pulses for single-qubit gates in a three-level transmon model with hardware-representative parameters. The main findings are:
...
The data that support the findings of this study are openly available in the QubitPulseOpt repository at \url{https://github.com/rylanmalarchick/QubitPulseOpt}. Raw simulation output files are located in the \texttt{results/} directory of the repository.
\end{acknowledgments}
