...
\begin{figure}[t]
    ...
    \end{minipage}
    \caption{Left: GRAPE optimization convergence showing fidelity improvement over iterations. The algorithm converges to near-unity fidelity within $\sim$200 iterations. Right: Comparison of pulse envelopes between a standard Gaussian pulse (orange) and the GRAPE-optimized pulse (blue), highlighting the complex, non-intuitive structure discovered by the optimizer. The GRAPE pulse is discretized into 50 time slices (shown interpolated for visualization); this non-smooth, rapidly-modulating structure is characteristic of optimal control solutions that exploit quantum interference effects.}
